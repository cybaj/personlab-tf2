...
class DataGenerator(object):
    def __init__(self, type='VALID'):  
        self.ANNO_FILE = config.VALID_ANNO_FILE
        self.IMG_DIR = config.VALID_IMG_DIR
        if type is 'TRAIN':
    ...
    def get_multi_scale_img(self,give_id,scale):
        img_id = give_id
        filepath = os.path.join(self.IMG_DIR,self.coco.imgs[img_id]['file_name'])
    ...
    def get_one_sample(self,give_id=None,is_aug=True):
        ...
        return [img.astype('float32')/255.0,kp_maps.astype('float32'),short_offsets.astype('float32'),
               ...
               crowd_mask.astype('float32'),unannotated_mask.astype('float32'),overlap_mask.astype('float32')], img_id 
                
    def gen_batch(self,batch_size=4):
        h,w,c = config.IMAGE_SHAPE
        while True:
            imgs_batch = np.zeros((batch_size,h,w,c), dtype='float32')
            kp_maps_batch = np.zeros((batch_size,h,w,config.NUM_KP), dtype='float32')
            short_offsets_batch = np.zeros((batch_size,h,w,2*config.NUM_KP), dtype='float32')
            mid_offsets_batch = np.zeros((batch_size,h,w,4*(config.NUM_EDGES)), dtype='float32')
            long_offsets_batch = np.zeros((batch_size,h,w,2*config.NUM_KP), dtype='float32')
            seg_mask_batch = np.zeros((batch_size,h,w,1), dtype='float32')
            crowd_mask_batch = np.zeros((batch_size,h,w,1), dtype='float32')
            unannotated_mask_batch = np.zeros((batch_size,h,w,1), dtype='float32')
            overlap_mask_batch = np.zeros((batch_size,h,w,1), dtype='float32')
            
            for i in range(batch_size):
                sample, img_id = self.get_one_sample()
            ...
            yield [imgs_batch,kp_maps_batch,short_offsets_batch,mid_offsets_batch,long_offsets_batch,
                  seg_mask_batch,crowd_mask_batch,unannotated_mask_batch,overlap_mask_batch], img_id
                    
    def get_coco_dt(self, dt_file):
        self.dt = self.coco.loadRes(dt_file)
        return self.dt
